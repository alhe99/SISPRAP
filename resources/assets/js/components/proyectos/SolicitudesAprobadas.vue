			...
			<div class="card-body">
				<div class="row">
					...
					<div v-if="proyecto_selectd != 0 && proyecto_selectd != null" class="col-md-10 col-sm-12 col-lg-6">
						<mdc-textfield type="text" style="margin-left: -10px" class="col-md-12"  @keyup="getSolicitudesAprobados(proyecto_selectd.value,1,buscar)"  label="Nombre del estudiante" v-model="buscar"></mdc-textfield>
					</div>
							...
							<tbody>
								<tr v-for="item in arrayPreregister" :key="item.id">
									...
									<td class="text-center">
										<button type="button" class="button red " @click="eliminarSolicitud(item.id,proyecto_selectd.value)" data-toggle="tooltip" title="Rechazar proyecto"><i class="mdi mdi-close"></i>&nbsp;Eliminar Aprobación</button>
									</td>
								</tr>
							</tbody>
						</table>
