		...
		<div class="card" v-if="proceso != 0 ">
			<div class="card-body">
				<div class="row">
					<div class="col-md-6 col-sm-12 col-lg-6  ">
						<h3 class=" align-baseline font-weight-bold" v-if="proceso == 1" >Listado de proyectos externos de Servicio Social</h3>
						<h3 class=" align-baseline font-weight-bold" v-if="proceso == 2">Listado de proyectos externos de Practica Profesional</h3>
					</div>
					<div class="col-md-12">
						<div class="row">
			...
			<div class="modal fade" :class="{'mostrar' : modalP }" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
										...
										<div v-if="estudiantesSelected.length == 0" class="row">
											<div class="col-md-12">
												<div class="alert alert-info font-weight-bold text-center" role="alert">
													Seleccione de la lista de estudiantes los que serán asignados a este proyecto externo
												</div>
											</div>
										</div>
										<div class="row">
											...
											<div class="col-md-12">
												<div class="row">
													...
													<div v-if="proceso==1" class="col-md-5"><br>
														<v-select v-model="nivelSelected" :options="arrayNiveles" placeholder="Nivel academico"></v-select>
											...
											<div v-if="proceso==1" class="col-md-10 col-sm-12 col-lg-6">
												<mdc-textfield v-if="nivelSelected != 0" type="text" style="margin-left: -10px" class="col-md-12"  @keyup="getEstudianteByCarrer(1,buscarP)"  label="Nombre del estudiante" v-model="buscarP"></mdc-textfield>
											</div>
											<div v-else class="col-md-10 col-sm-12 col-lg-6">
												<mdc-textfield v-if="carrera_selected != 0" type="text" style="margin-left: -10px" class="col-md-12"  @keyup="getEstudianteByCarrer(1,buscarP)"  label="Nombre del estudiante" v-model="buscarP"></mdc-textfield>
											</div>
											<div v-if="arrayEstudianteP.length != 0" class="col-md-12">
												<br><table class="table table-striped table-bordered table-mc-light-blue">
													...
													<tbody>
														<tr v-for="item in arrayEstudianteP" :key="item.id">
															...
															<td>
																<checkbox style="padding-left: 100px;" v-model="estudiantesSelected" :value="item.id">Asignar proyecto</checkbox>
															</td>
														</tr>
													</tbody>
												</table>
												...
												<div v-if="arrayEstudianteP.length == 0 && carrera_selected != 0 && nivelSelected != 0" class="col-md-12">
							...
							<div class="modal-footer">
								<div class="row">
									<div class="col-md-12">
										...
										<button type="button" :class="[estudiantesSelected.length == 0 ? 'disabled' : '']" :disabled="estudiantesSelected.length == 0" @click="asignarProyecto" class="button blue">Guardar Datos</button>
									</div>
								</div>
							</div>
						</div>
							...
							<tbody>
								<tr v-for="proyecto in arrayProyecto" :key="proyecto.id">
									...
									<td class="text-center">
										<button type="button" @click="abrirModal('proyecto','actualizar',proyecto)" class="button secondary" data-toggle="tooltip" title="Editar datos del Proyecto">
											<i class="mdi mdi-border-color"></i></button>
											<button type="button" @click="desactivarProyecto(proyecto.id)" class="button red" data-toggle="tooltip" title="Desactivar proyecto">
												<i class="mdi mdi-delete"></i>
											</button>
											<button class="button blue" @click="openModalProy(proyecto)" type="button" ta-toggle="tooltip"
											title="Asignar Proyecto"><i class="mdi mdi-bookmark-plus"></i></button>
										</td>
									</tr>
								</tbody>
							</table>
								...
								<div class="modal-dialog modal-lg" >
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<div class="row">
												<div class="col-md-12 col-xs-12 col-lg-12">
													<br><label  class="font-weight-bold" for="hrsRealizar">Horas a realizar</label>
													<input type="number" v-model="hrsRealizar" id="hrsRealizar" name="hrsRealizar" class="form-control" autocomplete="off" v-mask="'###'" min="0" :max="proceso==1 ? '300':'160'">
												</div>
											</div><br>
											<div class="form-group row">
												<div class="col-md-12 col-sm-12 col-lg-12">
				...
				export default {
					...
					methods: {
						...
						getEstudianteByCarrer(page) {
							let me = this;
							var url = "stundentByCarrer?page="+page+"&carrera_id="+me.carrera_selected.value+"&proceso_id="+me.proceso+
							"&nivelAcad="+me.nivelSelected.value+"&buscar=" + me.buscarP;
							me.loader = true;
							axios.get(url).then(function(response) {
								me.loader = false;
								var respuesta = response.data;
				...
				openModalProy(data = []){
					...
					if(this.proceso==2){this.nivelSelected= this.arrayNiveles[1]};
					this.getCarreras();
					this.modalP = 1;
				},
				...
				asignarProyecto(){
					swal({
						title: "Esta seguro de asignar este proyecto a los estudiantes seleccionados",
						type: "question",
						...
						if (result.value) {
							let me = this;
							me.loadSpinner = 1;
							var url = route('asinarProyectoExterno', {"estudent_id": me.estudiantesSelected,"project_id": me.proyecto_selected_id});
							axios.get(url)
							.then(function(response) {
								swal(
									"Hecho",
